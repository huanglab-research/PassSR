    ...
    def forward(self, x, time):
        ...
        for layer in self.downs:
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                x = layer(x)
            if k==5:
                high_fre=torch.nn.functional.interpolate(x_fre[:, 2:3, :, :], size=(128, 128), mode='bilinear', align_corners=False)
                x[:, 0:3, :, :]=x[:, 0:3, :, :]+high_fre
            if k==8:
                mid_fre=torch.nn.functional.interpolate(x_fre[:, 1:2, :, :], size=(64, 64), mode='bilinear', align_corners=False)
                x[:, 0:3, :, :]=x[:, 0:3, :, :]+mid_fre
            if k==11:
                low_fre=torch.nn.functional.interpolate(x_fre[:, 0:1, :, :], size=(32, 32), mode='bilinear', align_corners=False)
                x[:, 0:3, :, :]=x[:, 0:3, :, :]+low_fre
            k=k+1
            feats.append(x)
        k=0

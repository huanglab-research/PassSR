...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, x_in, continous=False):
        ...
        if not self.conditional:
            ...
            continuous_sqrt_alpha_cumprod = torch.FloatTensor(
                np.random.uniform(
                    self.sqrt_alphas_cumprod_prev[1999],
                    self.sqrt_alphas_cumprod_prev[2000],
                    size=1
                )
            ).to(x.device)
    ...
    def p_losses(self, x_in, noise=None):
        highpass_filter = torch.tensor([[[0, -1, 0], [-1, 5, -1], [0, -1, 0]],[[0, -1, 0], [-1, 5, -1], [0, -1, 0]],[[0, -1, 0], [-1, 5, -1], [0, -1, 0]]], dtype=torch.float32).view(1, 3, 3, 3)
        ...
        if not self.conditional:
            ...
            x_input=torch.cat((x_in['FRE'],x_in['SR'], x_noisy),1)#4,12,256,256
